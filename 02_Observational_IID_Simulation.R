...
# This scenario is for when we assume that individuals are IID
...
boot_ATE <- function(data, parametric){
  sample_ATE <- replicate(200, {
    ...
    sample_idx <- sample(1:nrow(data), size = 1000 ,replace = T)
...
system.time(results <- mclapply(1:n_jobs, function(i){
  param <- parameters[i, ]
  ATE_sim_one_obs(size_range[[param$size_idx]], param$para, param$ind)
}, mc.cores = 50)
)
